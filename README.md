# KLMS Plus — 学生生活ダッシュボード版

KLMS（Canvas LMS）のAccess Tokenで履修講義・時間割・課題を同期し、課題の期限状態、空き教室候補、提出準備をまとめて確認するWebアプリです。

## 追加された機能

### v1.4.1 ブラッシュアップ内容

- ダッシュボードに「期限超過」「課題完了率」「今の優先度」を追加
- PostgreSQL同期状態、シラバス取込件数、最終更新時刻をトップ画面に表示
- ヘッダーとカードUIを整理し、三層構造の提出物として見やすい画面に調整
- 配色と角丸を見直し、管理画面として落ち着いた印象に調整
...
課題管理画面のフィルターでは状態別に絞り込めます。ダッシュボードには「期限前1週間以内」「期限超過」「課題完了率」が表示されます。
...
- 課題を「期限超過済み」「期限前1週間以内」「期限前（8日以上）」「期限未設定」「完了済み」で分類
- 課題管理のプルダウンから期限状態ごとに絞り込み
- 慶應公式シラバス検索へのリンク
- 公式シラバス詳細URLから曜日・時限・キャンパス・教室を取り込み
- 曜日・時限ごとの空き候補／使用予定あり判定
- Render PostgreSQLへの保存・再読み込みを継続対応

既存のRenderデプロイへ反映する場合は、少なくとも `index.html`、`app.js`、`styles.css`、`server.js`、`package.json`、`package-lock.json` を更新してpushしてください。
